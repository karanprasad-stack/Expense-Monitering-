    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex space-x-3 sm:space-x-4">
          <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="flex-1 sm:flex-initial px-4 py-2 rounded-lg border border-gray-200 bg-white font-medium">
            {Array.from({length: 12}, (_, i) => i + 1).map(m => (
              <option key={m} value={m}>{new Date(0, m - 1).toLocaleString('default', { month: 'long' })}</option>
            ))}
          </select>
          <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="flex-1 sm:flex-initial px-4 py-2 rounded-lg border border-gray-200 bg-white font-medium">
            {[2024, 2025, 2026, 2027].map(y => (
              <option key={y} value={y}>{y}</option>
            ))}
          </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data?.recentExpenses && data.recentExpenses.map(exp => (
            <div key={exp._id} className="p-3.5 bg-gray-50 rounded-xl border border-gray-100 flex flex-col justify-between hover:bg-gray-100/50 transition-colors">
              <div>
                <div className="flex justify-between items-start">
                  <span className="font-semibold text-gray-800 text-sm truncate max-w-[150px]">{exp.description}</span>
