    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-gray-500">Record, edit, and monitor all your expenses</p>
        </div>
      </div>

      {/* Notifications / Alerts */}
      {warning && (
        <div className="p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl flex items-center justify-between shadow-sm">
          <div className="flex items-center space-x-2.5">
            <AlertCircle className="h-5 w-5 text-amber-600 flex-shrink-0" />
            <span className="font-semibold text-sm">{warning}</span>
          </div>
          <button onClick={() => setWarning('')} className="text-amber-600 hover:text-amber-800 text-xs font-bold p-1">
            ✕
          </button>
        </div>
      )}
      ...
      {errorMessage && (
        <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-2xl flex items-center justify-between shadow-sm">
          ...
          <button onClick={() => setErrorMessage('')} className="text-red-500 hover:text-red-700 text-xs font-bold p-1">
            ✕
          </button>
        </div>
      )}
      ...
      {successMessage && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl flex items-center justify-between shadow-sm">
          ...
          <button onClick={() => setSuccessMessage('')} className="text-emerald-600 hover:text-emerald-800 text-xs font-bold p-1">
            ✕
          </button>
        </div>
      )}
      ...
      <div className={`p-6 rounded-2xl shadow-sm border transition-all ${editingExpenseId ? 'bg-indigo-50/40 border-indigo-200 ring-2 ring-indigo-500/20' : 'bg-white border-gray-100'}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            {editingExpenseId ? (
              <>
                <Pencil className="h-5 w-5 text-indigo-600" />
                <h3 className="text-lg font-bold text-indigo-950">Edit Transaction</h3>
          ...
          {editingExpenseId && (
            <button
              ...
              className="text-xs text-gray-500 hover:text-gray-700 bg-white border border-gray-200 px-3 py-1.5 rounded-lg font-semibold transition-all hover:bg-gray-50"
            >
              Cancel Edit
            </button>
          )}
        ...
        <form onSubmit={handleFormSubmit} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white text-sm"
              value={date}
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white text-sm disabled:bg-gray-100 disabled:text-gray-400"
              value={subcategoryId}
          ...
          <div>
            ...
            <input
              ...
              required
              disabled={isSubmitting}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white text-sm font-semibold"
              value={amount}
          ...
          <div className="sm:col-span-2 md:col-span-3 flex flex-col sm:flex-row items-center gap-3 pt-2">
            <button
              type="submit"
              disabled={isSubmitting}
              className={`w-full sm:w-auto px-8 py-3 rounded-xl font-semibold shadow-md transition-all flex items-center justify-center space-x-2 ${
                isSubmitting
            ...
            {editingExpenseId && (
              <button
                ...
                onClick={handleCancelEdit}
                disabled={isSubmitting}
                className="w-full sm:w-auto px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-semibold transition-all"
              >
                Cancel
              </button>
            )}
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="relative min-w-[180px] flex-1 sm:flex-initial">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-1.5 rounded-lg border border-gray-200 text-xs focus:outline-none focus:ring-2 focus:ring-brand-500 bg-gray-50"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-lg border border-gray-200 text-xs bg-gray-50 text-gray-700"
            >
            ...
            {(searchTerm || selectedCategoryFilter || selectedSubcategoryFilter) && (
              <button
                ...
                className="p-1.5 text-xs text-gray-400 hover:text-gray-600 bg-gray-100 rounded-lg flex items-center gap-1 font-semibold"
                title="Clear Filters"
              >
        ...
        {filteredExpenses.length > 0 ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredExpenses.map((exp) => {
                  ...
                  return (
                    <tr
                      key={exp._id}
                      className={`hover:bg-gray-50/80 transition-colors ${isCurrentEditing ? 'bg-indigo-50/50 font-medium' : ''}`}
                    >
                      {/* Date */}
                      ...
                      <td className="py-3.5 pr-4">
                        <span className="font-semibold text-gray-800 block truncate max-w-[200px]">{exp.description || 'General'}</span>
                      </td>

                      {/* Category & Subcategory tags */}
                      <td className="py-3.5 pr-4">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-md font-bold text-[10px] uppercase tracking-wide">
                            {catName}
                          </span>
                          <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md font-bold text-[10px] uppercase tracking-wide">
                            {subName}
                          </span>
                        </div>
                      </td>
                      ...
                      {/* Payment Method */}
                      <td className="py-3.5 pr-4 text-xs text-gray-500 whitespace-nowrap">
                        <span className="text-[11px] bg-gray-50 text-gray-600 border border-gray-200/60 px-2 py-0.5 rounded-full font-medium">
                          {exp.paymentMethod || 'UPI'}
                        </span>
                      </td>

                      {/* Amount */}
                      <td className="py-3.5 pr-4 text-right whitespace-nowrap font-extrabold text-red-600">
                      ...
                      <td className="py-3.5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-1.5">
                          <button
                            ...
                            className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all font-semibold"
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg border border-gray-200 text-gray-500 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all font-semibold"
          ...
          <div className="py-12 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="text-sm font-medium text-gray-500">No transactions found matching your criteria.</p>
            {(searchTerm || selectedCategoryFilter) && (
              <button
                ...
                className="mt-2 text-xs text-brand-600 hover:underline font-bold"
              >
                Clear all filters
              </button>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-gray-100 transform scale-100 transition-all duration-200">
            <div className="flex flex-col items-center text-center space-y-4 mb-6">
              ...
              <div>
                <h4 className="text-lg font-bold text-gray-950">Delete Transaction?</h4>
                <p className="text-xs text-gray-500 mt-1">This will revert the spent amount in your budget.</p>
              </div>
            </div>

            <p className="text-sm text-gray-600 text-center mb-6">
              Are you sure you want to delete <span className="font-bold text-gray-800">"{deleteModal.description}"</span> (- ₹{deleteModal.amount})?
            </p>
